...
serve(async (req) => {
  try {
    ...
    const useCache = Boolean(UPSTASH_REDIS_REST_URL && UPSTASH_REDIS_REST_TOKEN);
    const cacheKey = `csv:consultation:${consultationId}`;
    if (useCache) {
      try {
        const cached = await fetch(`${UPSTASH_REDIS_REST_URL}/get/${encodeURIComponent(cacheKey)}`, {
    ...
    if (useCache) {
      try {
        const ttlSeconds = 60; // 1 minute cache; adjust as needed
        await fetch(`${UPSTASH_REDIS_REST_URL}/set/${encodeURIComponent(cacheKey)}/${encodeURIComponent(csv)}`, {
          method: 'POST',
          headers: {
