...
const Layout = ({ children }: LayoutProps) => {
  ...
  const handleSignOut = async () => {
    try {
      ...
      setTimeout(() => {
        window.location.replace('/'); // Force page reload without adding history
      }, 100);
    } catch (error) {
  ...
  return (
    <div className="relative min-h-screen bg-[#f7f8ff] dark:bg-slate-900 text-[#0f172a] dark:text-slate-100 overflow-hidden animate-page-smooth">
      ...
      <header className="relative z-10 border-b border-slate-200 dark:border-slate-700 bg-white/70 dark:bg-slate-900/70 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-slate-900/60">
        <div className="container mx-auto px-4 h-14 flex items-center justify-between">
          <Link to="/feed" replace className="text-lg font-bold text-slate-900 dark:text-slate-100">
            E-Consult
          </Link>
          ...
          <div className="flex items-center space-x-3">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-48 bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700" align="end" forceMount>
                <DropdownMenuItem onClick={() => navigate('/profile', { replace: true })} className="text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700">
      ...
      <main className="container mx-auto px-4 py-4 relative z-10 w-full min-w-0">
        {children}
      </main>
    </div>
  );
