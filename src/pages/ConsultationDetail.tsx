...
import { ArrowLeft, ArrowUpDown, Clock, MessageSquare, Send, Edit, Trash2, MoreHorizontal, Heart, X } from 'lucide-react';
import { api } from '@/services/apiClient';
import { useAuth } from '@/hooks/useAuth';
import { useToast } from '@/hooks/use-toast';
import { Alert, AlertDescription } from '@/components/ui/alert';
import { emitEvent, EVENTS } from '@/services/eventEmitter';
...
type CommentSort = 'recent' | 'older' | 'positive' | 'negative' | 'neutral';
...
const ConsultationDetail = () => {
  ...
  const fetchConsultation = async () => {
    try {
      const res = await api.getConsultation(id!);
      if (!res.item) throw new Error('Not found');
      setConsultation(res.item);
    } catch (error: any) {
  ...
  const subscribeToComments = () => {
    ...
    // Uses 5-second intervals with visibility detection
    const unsubscribe = smartPoll(
      async () => {
        try {
      ...
      5000, // Poll every 5 seconds (only when visible)
      true  // Start immediately on mount
    );
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const { item: inserted } = await api.postComment(id!, { content: newComment.trim() });
      ...
      if (inserted) {
        ...
        const nextComment: Comment = {
          id: inserted.id,
          content: inserted.content || newComment.trim(),
          created_at: inserted.created_at || new Date().toISOString(),
          user_id: inserted.user_id || user?.id || '',
          profiles: {
            display_name: inserted.profiles?.display_name || fallbackName,
            avatar_url: inserted.profiles?.avatar_url,
          },
          like_count: inserted.like_count ?? 0,
          liked_by_user: inserted.liked_by_user ?? false,
          sentimenttype: inserted.sentimenttype ?? null,
          score: inserted.score ?? null,
          keyword: inserted.keyword ?? null,
        };
      ...
      // Notify other pages that a comment was posted (for Feed/CommentedConsultations refresh)
      emitEvent(EVENTS.COMMENT_POSTED, { consultationId: id, commentId: inserted?.id });
  ...
  const handleEdit = async () => {
    ...
    try {
      await api.updateConsultation(consultation!.id, { title: editedTitle, description: editedDescription });
  ...
  const handleExpire = async () => {
    ...
    try {
      const res = await api.expireConsultation(consultation.id);
      ...
      // Update local state with backend-confirmed expiry
      setConsultation({ ...consultation, expires_at: res.item.expires_at });
  ...
  const getSentimentKey = (comment: Comment) => {
    const raw = String(comment.sentimenttype || '').toLowerCase();
    if (raw.includes('positive')) return 'positive';
    if (raw.includes('negative')) return 'negative';
    if (raw.includes('neutral') || raw.includes('netural')) return 'neutral';
    return 'unknown';
  };

  const getSortedComments = (list: Comment[]) => {
    const byDateDesc = (a: Comment, b: Comment) =>
      new Date(b.created_at).getTime() - new Date(a.created_at).getTime();
    const byDateAsc = (a: Comment, b: Comment) =>
      new Date(a.created_at).getTime() - new Date(b.created_at).getTime();

    if (commentSort === 'recent') return [...list].sort(byDateDesc);
    if (commentSort === 'older') return [...list].sort(byDateAsc);

    const preferred = commentSort;
    return [...list].sort((a, b) => {
      const aMatch = getSentimentKey(a) === preferred;
      const bMatch = getSentimentKey(b) === preferred;
      if (aMatch !== bMatch) return aMatch ? -1 : 1;
      return byDateDesc(a, b);
    });
  };

  // Export CSV handler - uses backend endpoint
  const handleExportCsv = async () => {
    try {
      if (!consultation?.id) {
        toast({ title: 'Error', description: 'Consultation not found' });
        return;
      }

      await api.exportCommentsCsv(consultation.id);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 p-4 animate-fade-in">
      ...
      <Card className="glass-card-pro bg-white/70 dark:bg-slate-800/60 border border-white/60 dark:border-white/10 backdrop-blur-xl border-l-4 border-l-primary">
          ...
          <div className="space-y-4">
            {isEditing ? (
              ...
              <>
                ...
                <CardTitle className="text-2xl font-bold leading-tight break-words text-slate-900 dark:text-slate-100">
                  {consultation.title}
                </CardTitle>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b pb-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">
                <ArrowUpDown className="w-3 h-3 mr-1" />
                <span className="text-xs">
                  Sort: {commentSort === 'recent' ? 'Recent' : commentSort === 'older' ? 'Older' : commentSort === 'positive' ? 'Positive' : commentSort === 'negative' ? 'Negative' : 'Neutral'}
                </span>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuRadioGroup value={commentSort} onValueChange={(value) => setCommentSort(value as CommentSort)}>
                <DropdownMenuRadioItem value="recent">Recent</DropdownMenuRadioItem>
                <DropdownMenuRadioItem value="older">Older</DropdownMenuRadioItem>
                <DropdownMenuRadioItem value="positive">Positive first</DropdownMenuRadioItem>
                <DropdownMenuRadioItem value="negative">Negative first</DropdownMenuRadioItem>
                <DropdownMenuRadioItem value="neutral">Neutral first</DropdownMenuRadioItem>
              </DropdownMenuRadioGroup>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
        ...
        <div className="space-y-4">
          {comments.length === 0 ? (
            ...
            sortedComments.map((comment, index) => (
              <div key={comment.id}>
                <Card className="glass-card-pro bg-white/70 dark:bg-slate-800/60 border border-white/60 dark:border-white/10 backdrop-blur-xl transition-none hover:translate-y-0">
                  <CardContent className="pt-6">
